...
- Cost caps: per-turn max tokens (30k in / 4k out), per-turn tool-step cap (5), concurrent-turn lock per project (also prevents parallel-request abuse) — numbers set as reversible engineering defaults in PROJECT_PLAN §17.1, since T9 cannot call the provider without them. The monthly provider budget and spend alerts are **deferred with approval** as a §24 exception (PROJECT_PLAN §17.1): nothing bounds a month's spend until it is configured, reviewed before the deployment is reachable by anyone but the owner. A per-user fair-use quota does not exist and is a pre-public-launch pricing decision.
