...
docs/VERTICAL_SLICE_TASKS.md in order; T1–T4 are done: scaffold and tooling, design tokens and appearance infrastructure, re-themed base primitives (review at /dev/primitives in development), and Supabase foundation (auth screens, projects with tested RLS). Connecting a real Supabase project requires the env vars in .env.example; RLS tests run with `DATABASE_URL=… npm run test:rls`.
